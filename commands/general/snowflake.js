...
class SnowflakeCommand extends Command {
  async run() {
    ...
    if (!this.args[0].match(/^<?[@#]?[&!]?\d+>?$/) || Number.parseInt(this.args[0]) < 21154535154122752n) return this.getString("commands.responses.snowflake.invalid");
    const id = Math.floor(((Number(this.args[0].replaceAll("@", "").replaceAll("#", "").replaceAll("!", "").replaceAll("&", "").replaceAll("<", "").replaceAll(">", "")) / 4194304) + 1420070400000) / 1000);
