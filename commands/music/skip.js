  ...
  async run() {
    ...
    if (player.host !== this.author.id && !this.memberPermissions.has("MANAGE_CHANNELS")) {
      const votes = skipVotes.get(this.guild.id) ?? {
        count: 0,
        ids: [],
        max: Math.min(3, player.voiceChannel.voiceMembers.filter((i) => i.id !== this.client.user.id && !i.bot).length),
      };
      ...
      if (votes.count + 1 === votes.max) {
        ...
        skipVotes.set(this.guild.id, {
          ...
          max: Math.min(
            3,
            player.voiceChannel.voiceMembers.filter((i) => i.id !== this.client.user.id && !i.bot).length,
          ),
        });
        this.success = true;
