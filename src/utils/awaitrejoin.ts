...
interface AwaitRejoinEvents {
  end: [member?: Member | { id: string; mention: string }];
}
...
class AwaitRejoin extends TypedEmitter<AwaitRejoinEvents> {
  ...
  constructor(channel: VoiceChannel | StageChannel, anyone: boolean, memberID: string) {
    ...
    this.checkInterval = setInterval(
      () => this.verify({ id: memberID, mention: `<@${memberID}>` }, channel, true),
      1000,
    );
  }

  verify(
    member: Member | { id: string; mention: string },
    channel: VoiceChannel | StageChannel | Uncached,
    checked = false,
  ) {
  ...
  stop(member?: Member | { id: string; mention: string }) {
